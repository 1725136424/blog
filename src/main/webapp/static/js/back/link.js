$(function () {
    // 数据表格
    $('#dg').datagrid({
        url: `${contextPath}/link/page`,
        columns: [[
            {field: 'name', title: '名称', width: 100, align: 'center'},
            {field: 'url', title: '请求地址', width: 100, align: 'center'},
        ]],
        fit: true,
        fitColumns: true,
        pagination: true,
        ...
        buttons: [
            {
                ...
                handler: function () {
                    ...
                    if ($("input[name=id]").val()) {
                        url = `${contextPath}/link/edit`
                    } else {
                        url = `${contextPath}/link/save`
                    }
    ...
    $(".delete").click(function () {
        $.messager.confirm("温馨提示", "是否删除", function (confirm) {
            if (confirm) {
                ...
                let selectedRow = $("#dg").datagrid("getSelected")
                $.ajax({
                    url: `${contextPath}/link/delete?id=` + selectedRow.id,
                    method: "get",
                    dataType: "json",
                    success: function (data) {
                        if (data.isSuccess) {
